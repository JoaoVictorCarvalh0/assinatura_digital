...
import { generateRSAKeyPair, sha256Hash, signHash, verifySignature } from './cryptoUtils.js';
  ...
  try {
    ...
    const user = await prisma.user.create({
      data: {
        name,
        email,
        password, // Em produção, use hash seguro!
        publicKey,
        privateKey,
      },
    });
...
router.get('/verify/:id', async (req, res) => {
  ...
  try {
    ...
    const valid = verifySignature(signature.hash, signature.signature, signature.user.publicKey);
    ...
    res.status(400).json({ error: 'Erro na verificação', details: err.message });
  }
});
